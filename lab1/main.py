...
async def parse_article(session, url):
    try:
        ...
        title = soup.find('h1') or soup.find('title')  # More generic
        content_elements = soup.find_all('p')
        category = soup.find('a', class_='rubric-label')  # Updated class
        created_date = soup.find('time')  # More generic
...
async def parse_main_page():
    async with aiohttp.ClientSession() as session:
        ...
        for a in soup.select('a'):  # More generic selection
            href = a.get('href', '')
            if href.startswith('/'):
                href = BASE_URL + href
